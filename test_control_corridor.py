...
class RobotTrainer:
    ...
    def select_action(self, state):
        ...
        return np.array([self.MAX_VEL[0], action[0]])

    def compute_reward(self, state, next_state):
        v = next_state[3] / self.MAX_VEL[0]  # Normalize linear velocity
        w = next_state[4] / self.MAX_VEL[1]  # Normalize angular velocity
        ...
        max_dist = np.sqrt(29)
        norm_dist_to_goal = dist_to_goal / max_dist
        ...
        # Reward function
        reward += v  # Encourage forward movement
        reward -= abs(w) ** 1.5  # Penalize excessive rotation
        reward -= norm_dist_to_goal ** 2  # Stronger penalty when far from goal
        reward -= 0.01  # Small penalty per step (prevents wandering)

        # Check boundary collision
        if np.abs(p[0]) < 0 or np.abs(p[0]) > 5 or np.abs(p[1]) > 1:
            reward -= 5  # Penalize hitting walls
            terminated = True

        # Define rectangular obstacles (xmin, ymin, xmax, ymax)
        obstacles = [
            (0.8, -1, 1.2, 0.3),  # Obstacle 1
            (1.8, -0.3, 2.2, 1),  # Obstacle 2
            (2.8, -1, 3.2, 0.3),  # Obstacle 3
            (3.8, -0.3, 4.2, 1)   # Obstacle 4
        ]
        
        # Collision check with rectangular obstacles
        for (xmin, ymin, xmax, ymax) in obstacles:
            if xmin <= p[0] <= xmax and ymin <= p[1] <= ymax:
                reward -= 10  # Strong penalty for hitting obstacles
                terminated = True
    ...
    def publish_velocity(self, action):
        ...
        vel_msg = Twist()
        vel_msg.linear.x = action[0]                           # Scale to actual velocity
    ...
    def training_loop(self, msg):
        ...
        if self.old_state is not None:
            self.replay_buffer.add(self.old_state, self.old_action[1], next_state, reward, float(done))
            
        # Train policy
        if self.replay_buffer.size > self.TRAINING_START_SIZE:
            self.policy.train(self.replay_buffer, batch_size=self.BATCH_SIZE)
            #rospy.sleep(1/8) # Frequency for ExpD3 64_128
    ...
    def evaluation(self, msg):
        ...
        action = self.policy.select_action(np.array(next_state))
        # Add random noise for exploration
        action += np.random.normal(0, self.expl_noise, size=self.ACTION_DIM)
        # Clip the angular velocity to be between -1 and 1
        action = np.clip(action, -1, 1)
        
        temp_action = np.array([self.MAX_VEL[0], action[0]])
        ...
        if not done:
            self.publish_velocity(temp_action)              # Execute action
            ##rospy.sleep(0.01)                             # Delay for Vicon measure
    ...
    def callback(self, msg):
        ...
        if  (self.total_training_time // 3600) > 40:
            print("EXITING. GOODBYE!")
            self.publish_velocity([0.0, 0.0])
            rospy.signal_shutdown("EXITING. GOODBYE!")
            return
